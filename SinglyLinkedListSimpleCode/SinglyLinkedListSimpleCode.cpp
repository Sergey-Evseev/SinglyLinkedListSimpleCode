...
int main()
{
	...
	lst.GetSize();//у списка можем вызвать геттер и посмотреть кол-во элементов
	cout << lst[2] << endl; //вывод значения элемента по его индексу

	int numbersCount; //переменная размеров списка для заполнения случай. значениями
	cin >> numbersCount;
	for (int i = 0; i < numbersCount; i++)//заполнение списка на numbersCount 'ktvtynjd
	{
