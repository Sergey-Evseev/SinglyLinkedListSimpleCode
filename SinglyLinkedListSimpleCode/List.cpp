...
void List<T>::insert(T data, int index)
{
	...
	else
	{
		...
		//либо без объявления новой переменной *newMode:
		//previous->pNext = new Node<T>(data, previous->pNext); - предыдущему указателю (ячейке) присвоить новые значения
...
{	
	Node<T> *temp = new Node <T>;//выделяем память под врем. переменную Node и присваиваем ее указателю
